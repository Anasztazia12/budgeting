...
function renderChart(incomes, expenses) {
	...
	const totalIncome = shared.sumEntries(incomes);
	if (totalIncome <= 0) {
	...
	const expenseEntries = Object.entries(expenseMap).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1]);
	...
	const remainingLabel = isHu ? "Maradék" : "Remaining";
	const incomeLabel = isHu ? "Bevétel" : "Income";
	...
	if (legendEl) {
		...
		incomeItem.className = "chart-legend-item";
		incomeItem.innerHTML = `
			<span class="chart-legend-color" style="background:#22c55e"></span>
			<span class="chart-legend-label"><strong>${incomeLabel}: ${formatCurrency(totalIncome)} (100%)</strong></span>
		`;
		legendEl.appendChild(incomeItem);

		expenseEntries.forEach(([label, amount], i) => {
			const item = document.createElement("div");
			item.className = "chart-legend-item";
			item.innerHTML = `
				<span class="chart-legend-color" style="background:${CHART_PALETTE[i % CHART_PALETTE.length]}"></span>
				<span class="chart-legend-label">${escapeHtml(label)}: <strong>${formatCurrency(amount)} (${pct(amount)})</strong></span>
			`;
		...
		if (remaining > 0) {
			...
			remItem.className = "chart-legend-item";
			remItem.innerHTML = `
				<span class="chart-legend-color" style="background:#d1d5db"></span>
				<span class="chart-legend-label">${remainingLabel}: <strong>${formatCurrency(remaining)} (${pct(remaining)})</strong></span>
			`;
			legendEl.appendChild(remItem);
		}
	}
	...
	if (titleEl) titleEl.textContent = isHu ? "Kiadások a bevétel arányában" : "Expenses as a share of income";
	...
	if (chartType === "pie") {
		const labels = [...expenseEntries.map(([k]) => k), ...(remaining > 0 ? [remainingLabel] : [])];
		const amounts = [...expenseEntries.map(([, v]) => v), ...(remaining > 0 ? [remaining] : [])];
		const bgColors = [
			...expenseEntries.map((_, i) => CHART_PALETTE[i % CHART_PALETTE.length]),
			...(remaining > 0 ? ["#d1d5db"] : [])
		];
			...
			chartInstance = new Chart(freshCanvas, {
				...
				options: {
					...
					plugins: {
						...
						tooltip: {
							callbacks: {
								label: (ctx) => ` ${ctx.label}: ${formatCurrency(ctx.raw)} (${pct(ctx.raw)})`
							}
						}
					}
				}
		...
		const labels = [incomeLabel, ...expenseEntries.map(([k]) => k)];
		const amounts = [totalIncome, ...expenseEntries.map(([, v]) => v)];
		const bgColors = ["#22c55e", ...expenseEntries.map((_, i) => CHART_PALETTE[i % CHART_PALETTE.length])];
					...
					plugins: {
						...
						tooltip: {
							callbacks: {
								label: (ctx) => {
									const val = ctx.raw;
									const percent = ctx.dataIndex === 0 ? "100%" : pct(val);
